...
export const recalculateShells: (shells: Shell[], ships: Ship[]) => Shell[] = (shells, ships) => {
  for(const shell of shells.values()) {
    ...
    const collide = ships.filter(s => {
      return v1.distance(new Victor(s.position.x, s.position.y)) < (COLLISION_DISTANCE/2)
    })
    if (collide.length > 0) {
